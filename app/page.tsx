...
const defaultStepPreparation:Record<string,StepPreparation>={
  ...
  "m4-buoc-03":{manual:["หากใช้ไฟล์ภายนอก ให้เตรียมลิงก์ Google Sheets ของลูกค้า เปิดสิทธิ์ให้บัญชีที่รัน Apps Script และบันทึก OPENAI_API_KEY ใน Script Properties"],automatic:["หากเว้น CUSTOMER_FILE_ID ว่าง ระบบจะอ่านชีต “ข้อมูลรายวัน” ในไฟล์ปัจจุบัน"]},
  ...
  "m4-buoc-07":{manual:["ก่อนเปิดทริกเกอร์ ให้กำหนด REPORTING_PERIOD เป็น “สัปดาห์ปัจจุบัน” และตรวจวันกับเวลาทำงานให้ถูกต้อง"],automatic:["ระบบจะตรวจข้อมูล คำนวณตัวชี้วัด สร้างร่างรายงาน AI และหยุดที่สถานะ “รออนุมัติ” โดยไม่ส่งอีเมลอัตโนมัติ"]},
...
const codeFiles2=[
  {n:"โค้ด 01",title:"ระบบหลัก เมนู และกระบวนการผลิต",where:"ลบโค้ดตัวอย่างใน Code.gs แล้ววางโค้ดทั้งหมดจาก Main.gs",path:"/files/model2/Main.gs",download:"Main.gs"},
  {n:"โค้ด 02",title:"ฝั่งเซิร์ฟเวอร์ของ Dashboard",where:"สร้างไฟล์สคริปต์ชื่อ WebApp แล้ววางโค้ดทั้งหมดจาก WebApp.gs",path:"/files/model2/WebApp.gs",download:"WebApp.gs"},
...
const codeFiles4=[
  {n:"โค้ด 01",title:"ระบบหลัก เมนู และกระบวนการรายงาน",where:"ลบโค้ดตัวอย่างใน Code.gs แล้ววางโค้ดทั้งหมด จากนั้นตั้งชื่อไฟล์เป็น Main.gs",path:"/files/model4/Main.gs",download:"Main.gs"},
  {n:"โค้ด 02",title:"ฝั่งเซิร์ฟเวอร์ของ Dashboard",where:"สร้างไฟล์สคริปต์ชื่อ WebApp แล้ววางโค้ดทั้งหมดจาก WebApp.gs",path:"/files/model4/WebApp.gs",download:"WebApp.gs"},
